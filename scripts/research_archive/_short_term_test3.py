# -*- coding: utf-8 -*-
"""短线买点优化 v3：次日开盘买 vs 信号日尾盘买（吃隔夜、避高开）
信号同 v2（放量突破 20 日均线 + 涨幅2~7% + 价>5 → top5/日）
卖出对比：3天到期 / TP4-SL2.5 / TP10-SL5-10天
组合引擎：100 万、20% 仓位/笔、T+1、5m 触发、到期收盘卖、每日收盘估值。
尾盘买入口径：信号日收盘价成交（≈14:55-15:00 尾盘决策）。
"""
import sqlite3
...
def main():
    ...
    def backtest(tp, sl, hold_days, buy_mode, tag):
        cash = INIT
        positions = {}
        nav_curve = []
        per_trade = []
        n_tp = n_sl = n_due = 0
        # 买入计划：buy_mode=close_signal -> 信号日当天尾盘买; open_next -> 次日开盘买
        plan = []
        for sig_day, tops in signal_days:
            si = day_idx[sig_day]
            if buy_mode == "close_signal":
                buy_day, buy_i = sig_day, si
            else:
                if si + 1 >= len(trade_days):
                    continue
                buy_day, buy_i = trade_days[si + 1], si + 1
            for c in tops:
                plan.append((buy_day, buy_i, c))
    ...
    print("=" * 20, "次日开盘买（原版对照）", "=" * 20)
    backtest(None, 0, 3, "open_next", "开盘/3天到期   ")
    backtest(0.04, 0.025, 3, "open_next", "开盘/TP4-SL2.5  ")
    backtest(0.10, 0.05, 10, "open_next", "开盘/TP10-SL5   ")
    print("=" * 20, "信号日尾盘买（新）", "=" * 20)
    backtest(None, 0, 3, "close_signal", "尾盘/3天到期   ")
    backtest(0.04, 0.025, 3, "close_signal", "尾盘/TP4-SL2.5  ")
    backtest(0.10, 0.05, 10, "close_signal", "尾盘/TP10-SL5   ")
