"""
技术面择时回测V7：ML Top20候选 + 金叉买入 + 死叉/止损/放宽放量滞涨止盈卖出
优化点（对比V1基线）：
1. 放宽放量滞涨止盈：仅在盈利>10%后启用
   - 放宽定义：成交量 > 5日均量2倍 + 收阴线 + 上影线长度 > 实体1.5倍
   - 只在盈利超过10%后才检查这个条件，避免盈利初期被洗出去
策略逻辑：
...
3. 卖出信号（满足任一即可）：
   a. 死叉：MA5下穿MA10
   b. 止损：亏损达5%
   c. 放宽放量滞涨止盈：盈利>10%后，出现放量滞涨（量>2倍+阴线+上影线>实体1.5倍）
...
# K线形态计算
df_bars['body'] = (df_bars['close'] - df_bars['open']).abs()  # 实体长度
df_bars['upper_shadow'] = df_bars['high'] - df_bars[['open', 'close']].max(axis=1)  # 上影线
df_bars['is_bearish'] = df_bars['close'] < df_bars['open']  # 收阴线

# 放宽放量滞涨：成交量>5日均量2倍 + 收阴线 + 上影线>实体1.5倍（V7放宽版）
df_bars['volume_surge_2x'] = df_bars['volume'] > df_bars['vol_ma5'] * 2
df_bars['long_upper_shadow_15x'] = (df_bars['body'] > 0) & (df_bars['upper_shadow'] > df_bars['body'] * 1.5)
df_bars['relaxed_volume_stagnation'] = (
    df_bars['volume_surge_2x'] & 
    df_bars['is_bearish'] & 
    df_bars['long_upper_shadow_15x']
)

print(f'  均线和K线形态计算完成, 耗时 {time.time()-t0:.1f}s')
print(f'  放宽放量滞涨信号数(全市场): {df_bars["relaxed_volume_stagnation"].sum()} 次')

# ========== 4. 合并因子和行情 ==========
print('合并因子和行情...')
df = df.merge(df_bars[['trade_date', 'code', 'ma5', 'ma10', 'ma20', 'vol_ma5',
                         'golden_cross', 'death_cross', 'volume',
                         'relaxed_volume_stagnation', 'body', 'upper_shadow', 'is_bearish']],
...
stagnation_count = df_candidates['relaxed_volume_stagnation'].sum()
print(f'  金叉信号: {golden_count} 次, 放宽放量滞涨信号(候选池): {stagnation_count} 次')

# ========== 6. 回测模拟（V7：放宽放量滞涨止盈，仅盈利>10%后启用）==========
print('开始回测模拟...')

INITIAL_CAPITAL = 1000000
MAX_HOLDINGS = 5
STOP_LOSS = 0.05              # 固定止损5%
PROFIT_THRESHOLD = 0.10       # 盈利超过10%后才启用放量滞涨止盈
COMMISSION = 0.0003
STAMP_DUTY = 0.0005
SLIPPAGE = 0.001
...
for i, date in enumerate(dates):
    ...
    for code, pos in holdings.items():
        ...
        if len(stock_data) > 0:
            ...
            # 卖出条件3：放宽放量滞涨止盈（仅盈利>10%后启用）
            elif (current_profit > PROFIT_THRESHOLD and 
                  pd.notna(row.get('relaxed_volume_stagnation')) and 
                  row['relaxed_volume_stagnation']):
                codes_to_sell.append((code, 'volume_stagnation'))
                volume_stagnation_profits.append(current_profit)
...
stop_loss_count = sell_reason_count.get('stop_loss', 0)
volume_stagnation_count = sell_reason_count.get('volume_stagnation', 0)
avg_vs_profit = np.mean(volume_stagnation_profits) if len(volume_stagnation_profits) > 0 else 0

# 保存结果
df_result.to_csv(f'{PROJECT}/artifacts/stock-ml/technical_timing_v7_backtest.csv', index=False)
df_trades.to_csv(f'{PROJECT}/artifacts/stock-ml/technical_timing_v7_trades.csv', index=False)

print(f'\n{"="*60}')
print(f'技术面择时V7回测结果（Top20候选 + 金叉买入 + 死叉/止损/放宽放量滞涨止盈，2025-06 ~ 2026-08）')
print(f'{"="*60}')
...
print(f'  止损卖出(亏5%):     {stop_loss_count}次')
print(f'  放宽放量滞涨止盈:   {volume_stagnation_count}次')
if volume_stagnation_count > 0:
    print(f'  放量滞涨止盈平均盈利: {avg_vs_profit:.2%}')
print(f'放量滞涨参数: 盈利阈值{PROFIT_THRESHOLD:.0%}, 量比>2倍, 阴线, 上影线>实体1.5倍')
print(f'总耗时:     {time.time()-t0:.1f}s')
print(f'\n结果已保存: artifacts/stock-ml/technical_timing_v7_backtest.csv')
print(f'交易记录:   artifacts/stock-ml/technical_timing_v7_trades.csv')
