"""
技术面择时回测V2：多因子Top20候选 + 金叉/放量突破双信号 + 死叉/5%止损
策略逻辑：
...
# ========== 3. 读取行情数据，计算均线和放量突破指标 ==========
print('读取行情数据并计算均线...')
df_bars = pd.read_sql_query(
    'SELECT trade_date, code, close, high, volume, amount FROM daily_bars WHERE trade_date >= "20250101" ORDER BY code, trade_date',
    mdb
)
...
# 20日最高价（不含当日，用于判断是否创20日新高）
df_bars['high_20d_prev'] = df_bars.groupby('code')['high'].transform(lambda x: x.shift(1).rolling(20).max())
...
# 放量突破：成交量 > 5日均量2倍 + 收盘价创20日新高 + 收盘价 > MA20
df_bars['volume_surge'] = df_bars['volume'] > df_bars['vol_ma5'] * 2
df_bars['breakout_20d'] = df_bars['close'] > df_bars['high_20d_prev']
df_bars['volume_breakout'] = df_bars['volume_surge'] & df_bars['breakout_20d'] & (df_bars['close'] > df_bars['ma20'])

print(f'  均线和放量指标计算完成, 耗时 {time.time()-t0:.1f}s')

# ========== 4. 合并因子和行情 ==========
print('合并因子和行情...')
df = df.merge(df_bars[['trade_date', 'code', 'ma5', 'ma10', 'ma20', 'vol_ma5',
                         'golden_cross', 'death_cross', 'volume', 'volume_breakout', 'high_20d_prev']],
...
breakout_count = df_candidates['volume_breakout'].sum()
both_count = (df_candidates['golden_cross'] & df_candidates['volume_breakout']).sum()
print(f'  金叉信号: {golden_count} 次, 放量突破信号: {breakout_count} 次, 同时满足: {both_count} 次')
...
for i, date in enumerate(dates):
    ...
    if len(holdings) < MAX_HOLDINGS:
        ...
        for _, row in buy_candidates.iterrows():
            ...
            # 买入信号：金叉 或 放量突破（满足任一即可）
            golden_signal = (pd.notna(row.get('golden_cross')) and row['golden_cross'] and
                           pd.notna(row.get('ma20')) and row['close'] > row['ma20'])
            breakout_signal = (pd.notna(row.get('volume_breakout')) and row['volume_breakout'])
            
            if golden_signal or breakout_signal:
                signal_type = 'golden_cross' if golden_signal else 'volume_breakout'
...
golden_buys = len(df_trades[(df_trades['action'] == 'buy') & (df_trades['signal'] == 'golden_cross')]) if len(df_trades) > 0 else 0
breakout_buys = len(df_trades[(df_trades['action'] == 'buy') & (df_trades['signal'] == 'volume_breakout')]) if len(df_trades) > 0 else 0

# 保存结果
df_result.to_csv(f'{PROJECT}/artifacts/stock-ml/technical_timing_v2_backtest.csv', index=False)
df_trades.to_csv(f'{PROJECT}/artifacts/stock-ml/technical_timing_v2_trades.csv', index=False)

print(f'\n{"="*60}')
print(f'技术面择时V2回测结果（金叉+放量突破双信号，2025-06 ~ 2026-08）')
print(f'{"="*60}')
...
print(f'买入次数:   {buy_count} (金叉{golden_buys}次, 放量突破{breakout_buys}次)')
print(f'卖出次数:   {sell_count}')
print(f'总耗时:     {time.time()-t0:.1f}s')
print(f'\n结果已保存: artifacts/stock-ml/technical_timing_v2_backtest.csv')
print(f'交易记录:   artifacts/stock-ml/technical_timing_v2_trades.csv')
