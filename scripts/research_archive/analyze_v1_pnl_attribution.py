"""
收益归因分析：seed=42模型的V1技术面择时回测
分析利润是否集中在少数股票/少数交易，分年度收益，胜率盈亏比等
"""
import sqlite3
...
if len(df_trades) > 0:
    ...
    # 最亏钱的股票
    print(f'\n--- 最亏钱的5只股票 ---')
    for code, row in stock_pnl.tail(5).iterrows():
    ...
    print(f'  最短: {df_trades["hold_days"].min()}天, 最长: {df_trades["hold_days"].max()}天')
...
# 保存交易记录
df_trades.to_csv(f'{PROJECT}/artifacts/stock-ml/v1_trade_analysis.csv', index=False)
print(f'\n交易记录已保存: artifacts/stock-ml/v1_trade_analysis.csv')
