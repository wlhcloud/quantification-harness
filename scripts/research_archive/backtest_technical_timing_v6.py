"""
技术面择时回测V6：ML Top20候选 + 金叉买入 + 死叉/止损/严格放量滞涨止盈卖出
优化点（对比V1基线）：
1. 严格放量滞涨止盈：仅在盈利>10%后启用
   - 严格定义：成交量 > 5日均量3倍 + 收阴线 + 上影线长度 > 实体2倍
...
3. 卖出信号（满足任一即可）：
   a. 死叉：MA5下穿MA10
   b. 止损：亏损达5%
   c. 严格放量滞涨止盈：盈利>10%后，出现放量滞涨（量>3倍+阴线+上影线>实体2倍）
...
# 严格放量滞涨：成交量>5日均量3倍 + 收阴线 + 上影线>实体2倍
df_bars['volume_surge_3x'] = df_bars['volume'] > df_bars['vol_ma5'] * 3
df_bars['long_upper_shadow'] = (df_bars['body'] > 0) & (df_bars['upper_shadow'] > df_bars['body'] * 2)
df_bars['strict_volume_stagnation'] = (
    df_bars['volume_surge_3x'] & 
    df_bars['is_bearish'] & 
    df_bars['long_upper_shadow']
)

print(f'  均线和K线形态计算完成, 耗时 {time.time()-t0:.1f}s')
print(f'  严格放量滞涨信号数: {df_bars["strict_volume_stagnation"].sum()} 次')

# ========== 4. 合并因子和行情 ==========
print('合并因子和行情...')
df = df.merge(df_bars[['trade_date', 'code', 'ma5', 'ma10', 'ma20', 'vol_ma5',
                         'golden_cross', 'death_cross', 'volume',
                         'strict_volume_stagnation', 'body', 'upper_shadow', 'is_bearish']],
...
stagnation_count = df_candidates['strict_volume_stagnation'].sum()
print(f'  金叉信号: {golden_count} 次, 严格放量滞涨信号: {stagnation_count} 次')

# ========== 6. 回测模拟（V6：严格放量滞涨止盈，仅盈利>10%后启用）==========
print('开始回测模拟...')
...
for i, date in enumerate(dates):
    ...
    for code, pos in holdings.items():
        ...
        if len(stock_data) > 0:
            ...
            # 卖出条件3：严格放量滞涨止盈（仅盈利>10%后启用）
            elif (current_profit > PROFIT_THRESHOLD and 
                  pd.notna(row.get('strict_volume_stagnation')) and 
                  row['strict_volume_stagnation']):
...
# 保存结果
df_result.to_csv(f'{PROJECT}/artifacts/stock-ml/technical_timing_v6_backtest.csv', index=False)
df_trades.to_csv(f'{PROJECT}/artifacts/stock-ml/technical_timing_v6_trades.csv', index=False)

print(f'\n{"="*60}')
print(f'技术面择时V6回测结果（Top20候选 + 金叉买入 + 死叉/止损/严格放量滞涨止盈，2025-06 ~ 2026-08）')
print(f'{"="*60}')
...
print(f'  严格放量滞涨止盈:   {volume_stagnation_count}次')
if volume_stagnation_count > 0:
    print(f'  放量滞涨止盈平均盈利: {avg_vs_profit:.2%}')
print(f'放量滞涨参数: 盈利阈值{PROFIT_THRESHOLD:.0%}, 量比>3倍, 阴线, 上影线>实体2倍')
print(f'总耗时:     {time.time()-t0:.1f}s')
print(f'\n结果已保存: artifacts/stock-ml/technical_timing_v6_backtest.csv')
print(f'交易记录:   artifacts/stock-ml/technical_timing_v6_trades.csv')
