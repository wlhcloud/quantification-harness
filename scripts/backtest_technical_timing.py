"""
技术面择时回测：多因子Top20候选 + MA5/MA10金叉死叉 + 5%止损
策略逻辑：
...
print(f'  均线计算完成, 耗时 {time.time()-t0:.1f}s')
...
# 股票池过滤：排除北交所(920)、科创板(688)，最小市值20亿
df_candidates = df_candidates[~df_candidates['code'].str.startswith(('920', '688'))]
df_candidates = df_candidates[df_candidates['mktCap'] >= 200000]  # 20亿（单位万元）
...
# 持仓状态
holdings = {}  # {code: {'shares': int, 'buy_price': float, 'buy_date': str}}
cash = INITIAL_CAPITAL
portfolio_values = []  # [(date, value, cash, holdings_count)]
    ...
    if len(holdings) < MAX_HOLDINGS:
        ...
        for _, row in buy_candidates.iterrows():
            ...
            if (pd.notna(row.get('golden_cross')) and row['golden_cross'] and
                ...
                # 等权分配
                target_value = (cash + sum(p['shares'] * df[(df['trade_date'] == date) & (df['code'] == c)]['close'].values[0] 
                              for c, p in holdings.items() if len(df[(df['trade_date'] == date) & (df['code'] == c)]) > 0)) / MAX_HOLDINGS
                buy_price = row['close'] * (1 + SLIPPAGE)
                shares = int(target_value / buy_price / 100) * 100  # 整手
...
# 保存结果（先保存，避免后续计算失败导致数据丢失）
df_result.to_csv(f'{PROJECT}/artifacts/stock-ml/technical_timing_backtest.csv', index=False)
print(f'\n结果已保存: artifacts/stock-ml/technical_timing_backtest.csv')
...
if len(df_hs300) > 0:
    benchmark_return = df_hs300['close'].iloc[-1] / df_hs300['close'].iloc[0] - 1
...
# 交易统计
print(f'\n{"="*50}')
print(f'技术面择时回测结果（2025-06 ~ 2026-08）')
print(f'{"="*50}')
